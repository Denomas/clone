...
impl BalloonPolicy {
    ...
    /// `mem_available_pct` is the guest's available memory as a percentage of total.
    pub fn report_activity(&mut self, active: bool) -> BalloonAction {
        self.report_activity_with_mem(active, 100.0)
    }

    /// Report guest activity with memory availability info.
    pub fn report_activity_with_mem(&mut self, active: bool, mem_available_pct: f64) -> BalloonAction {
        let now = Instant::now();
        ...
        match self.state {
            ...
            GuestState::Idle => {
                ...
                // Don't inflate if guest memory is tight (<25% available).
                // This prevents OOM-killing processes that are idle but
                // still holding allocated memory (e.g., Claude Code waiting
                // for input).
                if mem_available_pct < 25.0 {
                    return BalloonAction::Hold;
                }
