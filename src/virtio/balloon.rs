...
impl VirtioBalloon {
    ...
    pub fn process_inflate(&mut self, pfns: &[u32]) {
        const MMIO_HOLE_START: u64 = 0xC000_0000; // 3 GB
        const MMIO_HOLE_END: u64 = 0x1_0000_0000; // 4 GB
        ...
        for &pfn in pfns {
            ...
            #[cfg(target_os = "linux")]
            {
                ...
                let ret = unsafe {
                    libc::madvise(
                        self.guest_mem_base.add(host_offset as usize) as *mut libc::c_void,
                        BALLOON_PAGE_SIZE,
                        libc::MADV_DONTNEED,
                    )
                };
                if ret != 0 {
                    tracing::warn!(pfn, host_offset, "madvise(MADV_DONTNEED) failed");
                }
            }
