...
impl MmioTransport {
    ...
    pub fn read(&self, offset: u64, data: &mut [u8]) {
        ...
        let val: u32 = match offset {
            ...
            // SHM region registers: return -1 (no region available).
            // This tells the virtiofs driver that DAX is not supported,
            // so it falls back to the normal FUSE request queue path.
            reg::SHM_LEN_LOW | reg::SHM_LEN_HIGH => 0xFFFF_FFFF,
            reg::SHM_BASE_LOW | reg::SHM_BASE_HIGH => 0,
    ...
    pub fn write(&mut self, offset: u64, data: &[u8]) {
        ...
        match offset {
            ...
            reg::SHM_SEL => {
                // Guest selects SHM region ID. We don't have any, so reads
                // will return -1 for length regardless of which ID is selected.
            }
            reg::QUEUE_NUM => {
    ...
    /// an interrupt.
    /// Process descriptor chains for the given queue (public version for external callers).
    pub fn process_queue_descriptors_pub(&mut self, queue_idx: u16) {
        self.process_queue_descriptors(queue_idx);
    }
    ...
    fn process_queue_descriptors(&mut self, queue_idx: u16) {
        ...
        if qi >= self.virtqueues.len() {
            if self.device.device_type() == crate::virtio::DeviceType::Fs {
                // eprintln!("DBG: fs queue {} >= len {}", qi, self.virtqueues.len());
            }
            return;
        }
    ...
    pub fn restore_state(&mut self, state: &MmioTransportState) -> anyhow::Result<()> {
        ...
        // Sync Virtqueue objects from restored QueueState so the transport
        // can process descriptor chains for non-vhost devices.
        for (i, q) in self.queues.iter().enumerate() {
            if i < self.virtqueues.len() && q.ready {
                self.virtqueues[i].configure(q.desc_addr, q.avail_addr, q.used_addr);
                self.virtqueues[i].set_ready(true);
                // Sync last_avail_idx from guest memory avail ring
                self.virtqueues[i].sync_avail_from_guest();
            }
        }
        ...
        if state.activated && !self.activated {
            ...
            match self.device.activate() {
                Ok(()) => {
                    self.activated = true;
                    tracing::info!("Restored and activated virtio device");
                }
                Err(e) => {
                    tracing::error!("Failed to activate restored device: {e}");
                }
            }
        } else {
...
impl MmioBus {
    ...
    /// Reset a specific device by index (triggers full device re-probe by guest).
    /// Used after CoW fork to force the guest to drop stale vsock connections.
    pub fn reset_device(&mut self, index: usize) {
        if index < self.devices.len() {
            self.devices[index].force_reset();
        }
    }

    /// Reset all devices. Called during fork to force guest driver re-probe.
    ...
    /// Inject a VIRTIO_VSOCK_EVENT_TRANSPORT_RESET into the vsock device's
    /// event virtqueue. Called during fork to notify the guest that all
    /// existing vsock connections are dead and must be re-established.
    ///
    /// This writes directly to guest memory (the event virtqueue's descriptor
    /// buffers) before vCPUs start, so no synchronization is needed.
    pub fn inject_vsock_transport_reset(&self) {
        // MMIO hole: 0xC0000000..0x100000000 (1GB). GPAs above the hole
        // are mapped contiguously after the below-hole region.
        const MMIO_HOLE_START: u64 = 0xC000_0000;
        const MMIO_HOLE_SIZE: u64 = 0x4000_0000; // 1GB

        for transport in &self.devices {
            if transport.device.device_type() != crate::virtio::DeviceType::Vsock {
                continue;
            }
            if transport.queues.len() < 3 || transport.guest_mem.is_null() {
                continue;
            }

            let eq = &transport.queues[2];
            if eq.desc_addr == 0 || eq.avail_addr == 0 || eq.used_addr == 0 || eq.size == 0 {
                tracing::warn!("vsock transport reset: event queue not configured");
                return;
            }

            let mem = transport.guest_mem;

            // Translate GPA → host virtual address (accounting for MMIO hole)
            let gpa_to_hva = |gpa: u64| -> *mut u8 {
                let offset = if gpa >= MMIO_HOLE_START + MMIO_HOLE_SIZE {
                    (gpa - MMIO_HOLE_SIZE) as usize
                } else {
                    gpa as usize
                };
            ...
            let avail_idx: u16 = unsafe { *(gpa_to_hva(eq.avail_addr + 2) as *const u16) };
            let used_idx: u16 = unsafe { *(gpa_to_hva(eq.used_addr + 2) as *const u16) };

            if avail_idx == used_idx {
                tracing::warn!("vsock transport reset: no available buffers in event queue");
                return;
            }
            ...
            let buf_len: u32 = unsafe { *(gpa_to_hva(desc_gpa + 8) as *const u32) };

            if buf_len < 4 {
                tracing::warn!("vsock transport reset: buffer too small ({buf_len})");
                return;
            }
            ...
            unsafe {
                *(gpa_to_hva(used_entry_gpa) as *mut u32) = desc_idx as u32;
                *(gpa_to_hva(used_entry_gpa + 4) as *mut u32) = 4u32;
            }

            // Advance used.idx
            unsafe {
                *(gpa_to_hva(eq.used_addr + 2) as *mut u16) = used_idx.wrapping_add(1);
            }
    ...
    /// Get the IRQ number for the vsock device (for triggering after transport reset)
    pub fn vsock_irq(&self) -> Option<u32> {
        for transport in &self.devices {
            if transport.device.device_type() == crate::virtio::DeviceType::Vsock {
