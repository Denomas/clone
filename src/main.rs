...
enum Commands {
    ...
    Fork {
        ...
        /// Memory limit in MB (balloon inflates to reclaim excess)
        #[arg(long)]
        mem_mb: Option<u32>,

        /// Number of vCPUs to activate (remaining stay offline)
        #[arg(long)]
        vcpus: Option<u32>,

        /// Overlay tmpfs size (e.g. "10G")
        #[arg(long)]
    ...
    match cli.command {
        ...
        Commands::Fork {
            ...
            #[cfg(target_os = "linux")]
            {
                ...
                let mut vm = vmm::Vm::new(config)?;
                vm.fork_boot(&template, true, mem_mb, vcpus, overlay_size.as_deref())?;
                vm.run()?;
            }
            #[cfg(not(target_os = "linux"))]
            {
