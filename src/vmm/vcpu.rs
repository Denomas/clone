...
pub struct Vcpu {
    ...
    /// Deferred TSC_DEADLINE value to write right before first KVM_RUN.
    /// Written last to prevent the timer interrupt from being consumed
    /// by intervening ioctls (APICv PIR clearing).
    deferred_tsc_deadline: Option<u64>,
}
...
impl Vcpu {
    ...
    pub fn fd(&self) -> &kvm_ioctls::VcpuFd {
        &self.fd
    }
}
...
impl Vcpu {
    ...
    pub fn from_template(
        ...
        if false && !vcpu_state.cpuid.is_empty() {
            if let Ok(entries) = serde_json::from_slice::<Vec<(u32, u32, u32, u32, u32, u32, u32)>>(&vcpu_state.cpuid) {
                ...
                if let Ok(cpuid) = kvm_bindings::CpuId::from_entries(&cpuid_entries) {
                    ...
                    tracing::info!("Restored CPUID from snapshot ({} entries)", cpuid_entries.len());
                }
            }
        } else {
            // Fallback: generate from host (old snapshots without CPUID)
            let mut cpuid = kvm
                .get_supported_cpuid(KVM_MAX_CPUID_ENTRIES)
                .context("Failed to get supported CPUID")?;
            for entry in cpuid.as_mut_slice().iter_mut() {
        ...
        // Helper to restore a typed KVM struct from bytes
        fn restore<T: Copy>(data: &[u8], name: &str) -> Result<T> {
            let expected = std::mem::size_of::<T>();
            if data.len() != expected {
                anyhow::bail!("{name} size mismatch: got {}, expected {expected}", data.len());
            }
            Ok(unsafe { std::ptr::read(data.as_ptr() as *const T) })
        }

        // Restore order matches Firecracker exactly:
        // mp_state → regs → sregs → xsave → xcrs → debug_regs → lapic → msrs → vcpu_events → kvmclock_ctrl
        //
        // Critical ordering constraints (from Firecracker source):
        // - SET_LAPIC must come after SET_SREGS (SREGS restores APIC base MSR)
        // - SET_LAPIC must come before SET_MSRS (TSC_DEADLINE only works when LAPIC is configured)
        // - SET_REGS must come before SET_VCPU_EVENTS (SET_REGS clears pending exceptions)

        // 1. MP state — set to RUNNABLE so vCPU wakes from HLT
        let mp = kvm_bindings::kvm_mp_state { mp_state: 0 }; // RUNNABLE
        fd.set_mp_state(mp).context("set mp_state")?;

        // (probes removed — TSC_DEADLINE path abandoned, using one-shot LAPIC instead)

        // 2. General-purpose registers
        let regs: kvm_bindings::kvm_regs = restore(&vcpu_state.regs, "regs")?;
        fd.set_regs(&regs).context("set regs")?;

        // 3. Special registers (restores APIC base MSR)
        let sregs: kvm_bindings::kvm_sregs = restore(&vcpu_state.sregs, "sregs")?;
        fd.set_sregs(&sregs).context("set sregs")?;

        // 4. XSAVE
        if !vcpu_state.xsave.is_empty() {
            let expected = std::mem::size_of::<kvm_bindings::kvm_xsave>();
            if vcpu_state.xsave.len() == expected {
                let mut xsave: kvm_bindings::kvm_xsave = Default::default();
                unsafe {
                    std::ptr::copy_nonoverlapping(
                        vcpu_state.xsave.as_ptr(),
                        &mut xsave as *mut kvm_bindings::kvm_xsave as *mut u8,
                        expected,
                    );
                }
            ...
            fd.set_fpu(&fpu).context("set fpu")?;
        }

        // 5. XCRs
        if !vcpu_state.xcrs.is_empty() {
            let xcrs: kvm_bindings::kvm_xcrs = restore(&vcpu_state.xcrs, "xcrs")?;
            fd.set_xcrs(&xcrs).context("set xcrs")?;
        }

        // 6. Debug registers
        if !vcpu_state.debug_regs.is_empty() {
            let dregs: kvm_bindings::kvm_debugregs = restore(&vcpu_state.debug_regs, "debug_regs")?;
            fd.set_debug_regs(&dregs).context("set debug_regs")?;
        }
        ...
        if !vcpu_state.msrs.is_empty() {
            if let Ok(entries) = serde_json::from_slice::<Vec<(u32, u64)>>(&vcpu_state.msrs) {
                ...
                if !tsc_entries.is_empty() {
                    if let Ok(msrs) = Msrs::from_entries(&tsc_entries) {
                        let _ = fd.set_msrs(&msrs);
                    }
                }
            }
        }

        // 8. LAPIC — restore as-is from snapshot (one-shot mode with expired timer).
        // Then do a SECOND set_lapic with a fresh initial_count to restart the
        // one-shot timer. The kernel's timer handler will reprogram it.
        if !vcpu_state.lapic.is_empty() {
            let lapic: kvm_bindings::kvm_lapic_state = restore(&vcpu_state.lapic, "lapic")?;
            fd.set_lapic(&lapic).context("set lapic")?;
        }
        // 9. All MSRs (must come after LAPIC — TSC_DEADLINE needs LAPIC configured)
        // TSC_DEADLINE is deferred to right before KVM_RUN to prevent
        // APICv PIR clearing from consuming the timer interrupt.
        let mut deferred_tsc_deadline: Option<u64> = None;
        if !vcpu_state.msrs.is_empty() {
            if let Ok(mut entries) = serde_json::from_slice::<Vec<(u32, u64)>>(&vcpu_state.msrs) {
                ...
                for entry in entries.iter_mut() {
                    if entry.0 == 0x6E0 && entry.1 == 0 && tsc_val > 0 {
                        entry.1 = tsc_val;
                    }
                }

                // Skip TSC_DEADLINE MSR — using one-shot LAPIC timer instead.
                // Flag deferred write to restart the timer before KVM_RUN.
                deferred_tsc_deadline = Some(1);
                ...
                if let Ok(msrs) = Msrs::from_entries(&msr_entries) {
                    let written = fd.set_msrs(&msrs).unwrap_or(0);
                    tracing::info!("Restored {written}/{} MSRs (TSC_DEADLINE deferred)", msr_entries.len());
                }
            }
        }

        // 9. vCPU events (pending interrupts/exceptions)
        if !vcpu_state.vcpu_events.is_empty() {
            let events: kvm_bindings::kvm_vcpu_events = restore(&vcpu_state.vcpu_events, "vcpu_events")?;
            fd.set_vcpu_events(&events).context("set vcpu_events")?;
        }

        // 10. TSC frequency (if available and supported)
        if vcpu_state.tsc_khz > 0 {
            let _ = fd.set_tsc_khz(vcpu_state.tsc_khz);
        }

        // 11. Notify guest that vCPU was paused (prevents soft lockup watchdog)
    ...
    pub fn capture_state(&self) -> Result<crate::boot::template::VcpuState> {
        let regs = self.fd.get_regs().context("get kvm_regs")?;
        let sregs = self.fd.get_sregs().context("get kvm_sregs")?;
        let lapic = self.fd.get_lapic().context("get LAPIC")?;
        let fpu = self.fd.get_fpu().context("get FPU")?;
        let xsave = self.fd.get_xsave().context("get XSAVE")?;
        let xcrs = self.fd.get_xcrs().context("get XCRs")?;
        let mp_state = self.fd.get_mp_state().context("get MP state")?;
        let vcpu_events = self.fd.get_vcpu_events().context("get vCPU events")?;
        let debug_regs = self.fd.get_debug_regs().context("get debug regs")?;
        ...
        Ok(crate::boot::template::VcpuState {
            regs: Self::to_bytes(&regs),
            sregs: Self::to_bytes(&sregs),
            lapic: Self::to_bytes(&lapic),
            fpu: Self::to_bytes(&fpu),
            xsave: Self::to_bytes(&xsave),
            xcrs: Self::to_bytes(&xcrs),
            mp_state: Self::to_bytes(&mp_state),
            vcpu_events: Self::to_bytes(&vcpu_events),
            debug_regs: Self::to_bytes(&debug_regs),
            msrs: msrs_bytes,
            tsc_khz,
            cpuid,
        })
    ...
    /// Capture all MSRs from the vCPU. Uses KVM's msrs_to_save list
    /// (like Firecracker) to capture ALL MSRs the vCPU needs, not just
    /// a hardcoded subset.
    fn capture_msrs(&self) -> Result<Vec<u8>> {
        ...
        if !msr_indices.contains(&0x6E0) {
            msr_indices.push(0x6E0); // IA32_TSC_DEADLINE (not in KVM's list but needed for timer restore)
        }

        tracing::info!("Capturing {} MSRs (including TSC_DEADLINE)", msr_indices.len());
        ...
        for chunk in msr_indices.chunks(64) {
            ...
            if let Ok(mut msrs) = Msrs::from_entries(&entries) {
                if let Ok(count) = self.fd.get_msrs(&mut msrs) {
                    for entry in &msrs.as_slice()[..count] {
                        all_entries.push((entry.index, entry.data));
                    }
                }
            }
        }

        // Ensure TSC_DEADLINE is in the list (KVM may not return it via get_msrs).
        // Firecracker's fix_zero_tsc_deadline_msr handles the 0 case during restore.
        if !all_entries.iter().any(|&(idx, _)| idx == 0x6E0) {
            all_entries.push((0x6E0, 0));
            tracing::info!("TSC_DEADLINE not readable, added as 0 (fix_zero will handle)");
        }

        tracing::info!("Captured {} MSRs successfully", all_entries.len());
        Ok(serde_json::to_vec(&all_entries).unwrap_or_default())
    }
    ...
    pub fn run_loop(&mut self) -> Result<()> {
        ...
        // For forked VMs: inject NMI to force the kernel to re-evaluate
        // its timer state and restart the scheduler tick. The NMI handler
        // triggers timer recalibration on some kernel versions.
        // Only inject for forked VMs (detected by non-default LAPIC state).
        // Write deferred TSC_DEADLINE right before the first KVM_RUN.
        // This must be the LAST thing before entering the guest to prevent
        // Restart the LAPIC timer to fire immediately (one-shot mode).
        // This wakes the guest from HLT and lets hrtimer_interrupt reprogram.
        if self.deferred_tsc_deadline.take().is_some() {
            if let Ok(mut lapic) = self.fd.get_lapic() {
                ...
                if vector != 0 && ((lvt >> 17) & 3) == 2 {
                    ...
                    let new_lvt = vector;
                    lapic.regs[0x320..0x324].copy_from_slice(&new_lvt.to_le_bytes().map(|b| b as i8));
                }
                // Write a small initial_count to fire immediately
                let ic: u32 = 100;
                lapic.regs[0x380..0x384].copy_from_slice(&ic.to_le_bytes().map(|b| b as i8));
                let _ = self.fd.set_lapic(&lapic);
            }
        }

        let mut exit_count: u64 = 0;
        let mut serial_bytes: u64 = 0;
        let mut first_exits_logged = false;
        ...
        loop {
            ...
            // Check for pause on every exit (not just EINTR) so pausing
            // never waits for the vCPU to re-enter KVM_RUN and exit again.
            self.check_pause();
