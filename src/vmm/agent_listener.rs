...
pub enum AgentMessage {
    Heartbeat {
        ...
        #[serde(default = "default_mem_available")]
...
pub struct AgentState {
    ...
    /// Guest memory available percentage (from last heartbeat).
    pub mem_available_pct: Mutex<f64>,
...
impl AgentState {
    pub fn new() -> Self {
        Self {
            ...
            mem_available_pct: Mutex::new(0.0),
    ...
    pub fn send_exec(&self, command: &str, args: &[String]) -> Result<(i32, String, String), String> {
        ...
        loop {
            ...
            if timeout.is_zero() {
                self.exec_in_progress.store(false, Ordering::Release);
                return Err("Exec timed out after 120s".to_string());
            }
...
/// Start agent listener on a pre-connected unix socket fd (userspace vsock).
/// The fd is one end of a socketpair — the other end is inside the vsock device.
pub fn start_listener_fd(shutdown: Arc<AtomicBool>, fd: i32) -> Arc<AgentState> {
    let state = Arc::new(AgentState::new());
    let state_clone = Arc::clone(&state);
    ...
    std::thread::Builder::new()
        .name("agent-listener".into())
        .spawn(move || {
            listener_thread_fd(state_clone, shutdown, fd);
        })
        .expect("Failed to spawn agent listener thread");
...
fn listener_thread_fd(state: Arc<AgentState>, shutdown: Arc<AtomicBool>, fd: i32) {
    tracing::info!("agent-listener: listening on unix socket fd={fd}");

    // Read timeout must be longer than the agent's heartbeat interval (2s).
    // Use 10s to tolerate temporary stalls (exec commands, fork cleanup).
    ...
    // The unix socket is always "connected" (socketpair). We loop handling
    // the agent protocol. When the agent connects via vsock inside the guest,
    // the vsock device bridges data to this fd. When it disconnects, reads
    // return 0 or error, and we wait for the next connection.
    while !shutdown.load(Ordering::Relaxed) {
        tracing::info!("agent-listener: waiting for guest agent");

        // Wait for the agent to send its Ready message (first 4 bytes = length).
        ...
        *state.client_fd.lock().unwrap() = Some(fd);

        let (tx, rx) = mpsc::sync_channel::<AgentMessage>(16);
        *state.msg_rx.lock().unwrap() = Some(rx);

        // Process the first message we already started reading.
        ...
        // Continue handling this client.
        handle_client(&state, fd, &shutdown, &tx);

        // Client disconnected.
        *state.client_fd.lock().unwrap() = None;
        *state.msg_rx.lock().unwrap() = None;
        *state.mem_available_pct.lock().unwrap() = 0.0;
        state.connected.store(false, Ordering::Release);
        tracing::info!("agent-listener: guest agent disconnected");
    }

    // Don't close fd — it's owned by the VirtioVsock device.
}
...
fn listener_thread(state: Arc<AgentState>, shutdown: Arc<AtomicBool>, port: u32) {
    ...
    while !shutdown.load(Ordering::Relaxed) {
        ...
        tracing::info!("agent-listener: guest agent connected");
        state.connected.store(true, Ordering::Release);
        state.exec_in_progress.store(false, Ordering::Release); // Reset stale exec state
        ...
        *state.client_fd.lock().unwrap() = None;
        state.exec_in_progress.store(false, Ordering::Release); // Clear stale exec state
        *state.msg_rx.lock().unwrap() = None;
        *state.mem_available_pct.lock().unwrap() = 0.0;
        state.connected.store(false, Ordering::Release);
        tracing::info!("agent-listener: guest agent disconnected");
...
fn handle_client(state: &AgentState, fd: i32, shutdown: &AtomicBool, tx: &mpsc::SyncSender<AgentMessage>) {
    while !shutdown.load(Ordering::Relaxed) {
        ...
        let mut len_buf = [0u8; 4];
        match read_exact_result(fd, &mut len_buf) {
            ReadResult::Ok => {}
            ReadResult::Timeout => continue, // SO_RCVTIMEO fired, retry
            ReadResult::Disconnected => return,
        }
        ...
        match read_exact_result(fd, &mut body) {
            ReadResult::Ok => {}
            ReadResult::Timeout | ReadResult::Disconnected => {
                // Header was already consumed. If the body times out or disconnects,
                // the stream is desynchronized — we must reconnect.
                return;
            }
        }
            ...
            Ok(msg) => {
                ...
                } = &msg
                {
                    ...
                    *state.mem_available_pct.lock().unwrap() = *mem_available_pct;
                    ...
                    if *ping_id > 0 {
                        let _ = send_vmm_message(fd, &VmmMessage::Pong { ping_id: *ping_id });
                    }
                    tracing::debug!(
                        active,
                        load_avg_1m,
...
fn read_exact(fd: i32, buf: &mut [u8]) -> bool {
    matches!(read_exact_result(fd, buf), ReadResult::Ok)
}
